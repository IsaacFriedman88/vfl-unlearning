...
from common_nih_chest_xray14_vfl import CFG, PartialImageEncoder, ServerBinaryClassifier, get_nih_loaders
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a local two-party VFL experiment on NIH Chest X-ray14 with SecretFlow-style setup."
    )
...
def main() -> None:
    ...
    print(f"Running NIH Chest X-ray14 VFL from SecretFlow folder on {device} with task '{args.task_label}'.")
    print(
    ...
    append_run_summary(
        ...
        notes=(
            f"Task={args.task_label}, SecretFlow mirror, two-party vertical split: left_half vs right_half; "
            f"seed={args.seed}"
        ),
    )
