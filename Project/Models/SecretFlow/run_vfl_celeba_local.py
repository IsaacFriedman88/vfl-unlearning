...
from common_celeba_vfl import AttributeParty, CFG, ImageEncoder, ServerHead, get_celeba_loaders
from run_logging import append_csv_row, append_run_summary, create_run_log_paths, init_metrics_log, write_latest_pointer
from secretflow_vfl_shared import validate_secretflow_runtime
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a local two-party VFL experiment on CelebA with SecretFlow-style setup.")
    ...
    parser.add_argument("--skip-secretflow-check", action="store_true")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    if not args.skip_secretflow_check:
        validate_secretflow_runtime()

    seed_everything(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    train_loader, test_loader = get_celeba_loaders(
        root=args.data_root,
    ...
    log_paths = create_run_log_paths("celeba")
    metric_fields = [
        "phase",
        "epoch",
        "active_loss",
    ...
    left_party = ImageEncoder().to(device)
    right_party = AttributeParty().to(device)
    server_head = ServerHead().to(device)

    optimizer = create_optimizer(left_party, right_party, server_head, lr=CFG.LR)

    print("Running CelebA VFL from SecretFlow folder on", device)
    print("Left party: image encoder | Right party: sensitive attribute Eyeglasses | Task: predict Smiling")
    print(
    ...
    append_run_summary(
        ...
        notes=(
            f"Task=Smiling, attribute_party=Eyeglasses, SecretFlow mirror; "
            f"seed={args.seed}"
        ),
    )
