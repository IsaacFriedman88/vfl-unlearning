...
from common_mnist_vfl import CFG, PartialImageEncoder, ServerClassifier, get_mnist_loaders
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a local two-party VFL experiment on MNIST with SecretFlow-style setup.")
    ...
    parser.add_argument(
        "--skip-secretflow-check",
        action="store_true",
        help="Allow local smoke runs even when secretflow is not installed.",
    )
...
def main() -> None:
    ...
    log_paths = create_run_log_paths("mnist")
    metric_fields = [
        "phase",
        "epoch",
        "active_loss",
    ...
    server_head = ServerClassifier().to(device)

    optimizer = create_optimizer(left_party, right_party, server_head, lr=CFG.LR)

    print(f"Running MNIST VFL from SecretFlow folder on {device}.")
    print(
    ...
    append_run_summary(
        ...
        notes=(
            f"SecretFlow mirror, two-party vertical split: left_half vs right_half; "
            f"seed={args.seed}"
        ),
    )
