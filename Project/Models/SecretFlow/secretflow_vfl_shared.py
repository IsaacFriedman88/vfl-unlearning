...
class SecretFlowDependencyError(RuntimeError):
    """Raised when the requested SecretFlow runtime is unavailable."""


def validate_secretflow_runtime() -> None:
    try:
        __import__("secretflow")
    except ModuleNotFoundError as exc:
        raise SecretFlowDependencyError(
            "SecretFlow is not installed in the active environment. "
            "Create a supported Python environment with secretflow, torch, torchvision, "
            "pandas, scikit-learn, and pillow before running these experiments."
        ) from exc
